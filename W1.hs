...
hassuMinimi a b = case hassuCompare a b of LT -> a
                                           GT -> b
                                           EQ -> b
...
pyramidi 0 = "0"
pyramidi n = (joinStr list) ++ "," ++ (joinStr $ tail $ reverse list)
  where list = map show [0..n]
        joinStr xs = intercalate "," xs
...
nextPrime 0 = 0
nextPrime 1 = 1
nextPrime n = head [x | x <- [n..], isPrime x]
